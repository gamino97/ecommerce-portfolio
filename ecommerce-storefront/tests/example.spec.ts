...
test('testCreateProduct', async ({ page }) => {
  ...
  await page.locator('input[name="price"]').fill(faker.commerce.price({ min: 1, max: 1000 }));
  await page.getByRole('spinbutton', { name: 'Stock' }).click();
  await page.getByRole('spinbutton', { name: 'Stock' }).fill(faker.number.int({ min: 1, max: 1000 }).toString());
  await page.getByRole('combobox', { name: 'Category' }).click();
  await page.getByLabel('Books').getByText('Books').click();
  await page.getByRole('textbox', { name: 'Description' }).click();
  await page.getByRole('textbox', { name: 'Description' }).fill(faker.commerce.productDescription());
